...
from rpy2 import robjects # to use R for fitting a and b parameters in beta binomial 
...
#parser.add_argument('--min_nonzerocell_per_cluster', dest='output', 
#                    help='name of the output file to save junction count matrix to')
...
#clust_file="/gpfs/commons/groups/knowles_lab/Karin/parse-pbmc-leafcutter/leafcutter/junctions/out.txt.gz"
clust_file="/gpfs/commons/groups/knowles_lab/Karin/zoo-brain/leafcutter_pipeline/junctions/Pig/pig_out.txt.gz"
...
def get_missing_junctions(clust, cell):
...
def get_junc_beta_params(clust, cell):
    '''
    ...
    clust_matrix=get_missing_junctions(clust, cell) #do this for every cell type
...
def main():
    ...
    pool.starmap(get_junc_beta_params, zip(all_clusters, itertools.repeat(cell_types)))
