...
class CustomDataset(Dataset):
    def __init__(self, junction_tensor, cluster_tensor):
        ...
        self.juncratios = self.junction_tensor/self.cluster_tensor
    def __len__(self):
    ...
    def __getitem__(self, idx):
        ...
        juncratio_data = self.juncratios[idx]
        return junction_data, cluster_data, juncratio_data
...
def setup_data_loaders(junction_tensor, cluster_tensor, batch_size=128, use_cuda=False):
    ...
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, generator=torch.Generator(device="cuda" if use_cuda else "cpu"))
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, generator=torch.Generator(device="cuda" if use_cuda else "cpu"))
